...
def count_homonymy(fname, root):
    ...
    for line in text_file:
        ...
        if token != None:
            tokens += 1
        tag = re.findall('<ana[^>]*></ana>', line)
        tags += len(tag)
        homonymy = re.search('(<ana[^>]*></ana>){2,}', line, flags = re.U)
        if homonymy != None:
            number_of_homo += 1
...
##        trigr_stat.write(trigr + ';' + str(trigr_count[trigr]) + '\r\n')
    ...
    bigr_stat = codecs.open('Bigrams_statistics.csv', 'w', 'utf-8-sig')
